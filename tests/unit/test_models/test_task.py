...
def test_task_creation(create_user):
    """Prueba crear una tarea válida."""
    user = create_user
    task = Task(content="Test Task", priority="High", completed=False, user_id=user.id)
    db.session.add(task)
    db.session.commit()
...
def test_task_default_values(create_user):
...
def test_task_missing_content(create_user):
...
def test_task_repr(create_user):
